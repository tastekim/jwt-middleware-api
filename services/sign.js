...
router.post("/signup", async (req, res) => {
    try {
        ...
        if (!(/^[a-zA-Z0-9]{3,}$/).test(nickname)) {
            return res.status(401).json({errorMessage: "nickname 양식을 지켜주세요."});
        }

        // password 조건 확인.
        if (password.search(nickname) > -1) {
            return res.status(401).json({errorMessage: "nickname 을 password 에 포함할 수 없습니다."});

        }

        // 비밀번호 일치 여부 확인.
        if (password !== confirmPassword) {
            return res.status(401).json({errorMessage: "패스원드가 패스워드확인과 일치하지 않습니다."});
        }
        ...
        if (existUsers.length > 0) {
            return res.status(401).json({errorMessage: "이미 존재하는 닉네임 입니다."});
        }

        // 유저 정보 생성.
        await Users.create({nickname, password});
        res.status(200).json({message: "회원가입에 성공했습니다 ! 로그인을 시도해주세요 !"});
    } catch (err) {
        console.log(err);
